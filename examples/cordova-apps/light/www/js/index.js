...
var app = {
    ...
    bindEvents: function() {
        ...
        bulb.addEventListener('touchstart', this.onToggle, false);                                          // if bulb image touched, goto: onToggle
    },
    onDeviceReady: function() {
        ...
        document.querySelector("img").className = "Dis";                                                    // bulb image in 'disconnected' mode
    ...
    onDiscover: function(device) {
        if (device.id == deviceID) {
            document.querySelector("#console").innerHTML = "Found! Connecting...";
            ble.connect(deviceID, app.onConnect, app.onDeviceReady);                                        // if device matches, connect; if connected, goto: onConnect
        }
    },
    onConnect: function(device) {
        document.querySelector("#console").innerHTML = "Connected! Syncing...";
    ...
    onRead: function(data) {
        document.querySelector("img").className = ((new Uint8Array(data))[0]===0x1) ? "On" : "Off";         // bulb image in 'on' or 'off' mode
        document.querySelector("#console").innerHTML = document.querySelector("img").className;             // display text: "On" or "Off"
    },
    onToggle: function(event) {
        if (event.target.className!="Dis") {                                                                // if image touched & bulb connected,
            data = new Uint8Array(1);
            data[0] = (event.target.className=="Off") ? 0x1 : 0x0;                                          // set up switch toggle write 
