var PRMISN = {"cordova-plugin-ble-central":"Bluetooth","com.megster.cordova.ble":"Bluetooth","cordova-plugin-device":"Device Information","cordova-plugin-file":"File Read/Write","cordova-plugin-network-information":"Network Information","cordova-plugin-zeroconf":"Bonjour/mDNS","cordova-plugin-zip":"ZIP Compression","bluetooth":"Bluetooth","battery":"Battery Information","device":"Device Information","globalization":"Locale Information","connection":"Network Information","accelerometer":"Accelerometer","camera":"Camera","compass":"Compass","geolocation":"Geolocation","capture":"Media Capture","vibrate":"Vibrate","contacts":"Contacts","file":"File Upload/Download","media":"Media View","notification":"Dialogs","zeroconf":"Bonjour/mDNS","zip":"ZIP Compression"};
...
var app = {
  initialize: function() {
    ...
    $("#panel .pa").click(function(e){e.preventDefault();window.open($(this).attr("href"),'_system');return false;});
  ...
  onAppReady: function() {
    ...
    app.bookmarks = app.getStoredObject("bookmarks")||{};
    ...
    $("body").pagecontainer({ beforeshow: function(e,ui) { if (ui.toPage.attr("id")=="page4") app.loadFaves(); } });
  },
  onPause: function() {
  ...
  onPrefChange: function() {
    ...
    } else if ($(this).attr("id")=="so") {
      localStorage.setItem("so",$(this).val());
      $("#devs").html($("#devs li").sort(app.sortRule)).listview("refresh");
    } else {
    ...
    if (peripheral.action=="type") cordova.plugins.zeroconf.watch(peripheral.service.qualifiedname,app.onDiscover);
      ...
      if (typeof app.peripherals[peripheral.service.qualifiedname] == "undefined") {
        ...
        if (peripheral.service.type.startsWith("_http._tcp.")) {
          $('li.other[data-id="'+peripheral.service.qualifiedname+'"]').remove(); 
          ...
          if ($('li[data-id="'+peripheral.service.qualifiedname+'"]:not(.other)').length==0) $("#other").before($("<li>",{class:"nsd item","data-url":peripheral.uri,"data-rssi":time*-1e-10,"data-pplr":time,"data-time":time,"data-id":peripheral.service.qualifiedname}).append($("<a>",{href:"#",style:"opacity:.5; pointer-events:none",onclick:"app.go('"+peripheral.service.qualifiedname+"')"}).append($("<img>",{src:"img/dnssd.svg"})).append($("<h2>").html(peripheral.service.name)).append($("<p>").html(peripheral.uri+"<br/><i class='zmdi zmdi-network-wifi-alt zmd-fw'></i> "+(peripheral.service.server||peripheral.service.hostName)+" ("+(peripheral.service.application||peripheral.service.type)+")"))));
          if ( (url=app.urls[peripheral.uri]) && (meta=app.meta[url].meta) ) app.attach(peripheral,meta);
            ...
            $.post("https://summon-caster.appspot.com/resolve-scan",JSON.stringify({objects:[{url:peripheral.uri}]}),function(data){
              ...
              else $('li[data-id="'+peripheral.service.qualifiedname+'"]').html($("<a>",{href:'#',onclick:"app.go('"+peripheral.service.qualifiedname+"')"}).append($("<img>",{src:peripheral.uri+"/favicon.ico"}).error(function(){$(this).attr("src","img/dnssd.svg")})).append($("<h2>").html(peripheral.service.name)).append($("<p>").html(peripheral.uri+"/<br/><i class='zmdi zmdi-network-wifi-alt zmd-fw'></i> "+(peripheral.service.server||peripheral.service.hostName)+" ("+(peripheral.service.application||peripheral.service.type)+")"))).append($("<a>",{href:"#dialog","data-rel":"popup","data-transition":"pop",class:"zmdi zmdi-more-vert",onclick:"app.infoPopup('"+peripheral.service.qualifiedname+"','nsd')"})).append("<div class='zmdi zmdi-star"+(app.bookmarks[peripheral.uri]?" fav":"")+" zmd-lg' style='float:right; position:relative; z-index:1000' onclick='app.bookmark(\""+peripheral.uri+"\")'></div>");
              $("#devs").html($("#devs li").sort(app.sortRule)).listview("refresh");
            }).fail(function(e){
              $('li[data-id="'+peripheral.service.qualifiedname+'"]').html($("<a>",{href:'#',onclick:"app.go('"+peripheral.service.qualifiedname+"')"}).append($("<img>",{src:peripheral.uri+"/favicon.ico"}).error(function(){$(this).attr("src","img/dnssd.svg")})).append($("<h2>").html(peripheral.service.name)).append($("<p>").html(peripheral.uri+"/<br/><i class='zmdi zmdi-network-wifi-alt zmd-fw'></i> "+(peripheral.service.server||peripheral.service.hostName)+" ("+(peripheral.service.application||peripheral.service.type)+")"))).append($("<a>",{href:"#dialog","data-rel":"popup","data-transition":"pop",class:"zmdi zmdi-more-vert",onclick:"app.infoPopup('"+peripheral.service.qualifiedname+"','nsd')"}));
              $("#devs").html($("#devs li").sort(app.sortRule)).listview("refresh");
            });
          }
        } else {
          $(".other:last-child").hide().after($("<li>",{class:"other nsd item","data-icon":"false",'data-id':peripheral.service.qualifiedname,"data-rssi":time*-1e-9,"data-pplr":time*1.01,"data-time":time*1.01}).hide().html($("<a>",{href:"#page2","data-transition":"slide","onclick":"app.generate('"+peripheral.service.qualifiedname+"')"}).html("<i class='zmdi zmdi-network-wifi-alt zmd-fw'></i> "+peripheral.service.name+" ("+(peripheral.service.application||peripheral.service.type)+")")));
  ...
  //     n = payload;
  //     (function(n){
  //       $("#other").before($("<li>",{class:"nfc item","data-rssi":-200}).append($("<a>",{href:payload}).append($("<img>",{src:"img/dnssd.svg"})).append($("<h2>").html(payload)).append($("<p>").html(payload+"<br/>"+payload+" ("+payload+")"))).append($("<a>",{href:"#dialog","data-rel":"popup","data-transition":"pop",class:"zmdi zmdi-more-vert",onclick:"app.infoPopup(n,'nfc')"})));
  //     })(n);
  //     $.mobile.loading("hide");
  ...
  onPeripheralFound: function(peripheral) {
    ...
    if (peripheral.uri) {
      $('li.other[data-id="'+peripheral.id+'"]').remove(); 
      if ($('li[data-id="'+peripheral.id+'"]').length==0 && !app.urls[peripheral.uri]) {
        $("#other").before($("<li>",{class:"ble item","data-id":peripheral.id,"data-rssi":peripheral.rssi,"data-pplr":Date.now(),"data-time":Date.now()}).append($("<a>",{href:'#',style:"opacity:.5; pointer-events:none"}).append($("<img>",{src:"img/ble.svg"})).append($("<h2>").html(peripheral.name+" ("+peripheral.id+")")).append($("<p>").html(peripheral.uri+"<br/>"+"<i class='zmdi zmdi-bluetooth zmd-fw'></i> "+peripheral.name+" ("+peripheral.id+")"))));
      }
      if (peripheral.uri != "local") {
        if ( (url=app.urls[peripheral.uri]) && (meta=app.meta[url].meta) ) app.attach(peripheral,meta);
          ...
          $.post("https://summon-caster.appspot.com/resolve-scan",JSON.stringify({objects:[{url:peripheral.uri}]}),function(data){
            if (data.metadata[0]) app.attach(peripheral,data.metadata[0]);
            else $('li[data-id="'+peripheral.id+'"]').addClass("ble").html($("<a>",{href:'#',onclick:"app.go('"+peripheral.id+"')"}).append($("<img>",{src:peripheral.uri+"/favicon.ico"}).error(function(){$(this).attr("src","img/ble.svg")})).append($("<h2>").html(peripheral.name+" ("+peripheral.id+")")).append($("<p>").html(peripheral.uri+"<br/>"+"<i class='zmdi zmdi-bluetooth zmd-fw'></i> "+peripheral.name+" ("+peripheral.id+")"))).append($("<a>",{href:'#dialog',"data-rel":'popup',"data-transition":"pop",onclick:"app.infoPopup('"+peripheral.id+"','ble')"}));
            $("#devs").html($("#devs li").sort(app.sortRule)).listview("refresh");
          }).fail(function(e){
            if (typeof app.cachelist[peripheral.id] != "undefined" && typeof (app.cachelist[peripheral.id]).meta != undefined) app.attach(peripheral,(app.cachelist[peripheral.id]).meta)
            else if(peripheral.uriFrame==0xFF) $('li[data-id="'+peripheral.id+'"]').addClass("ble").html($("<a>",{href:'#',onclick:"app.uiLoad('"+peripheral.id+"');"}).append($("<img>",{src:"img/ble.svg"})).append($("<h2>").html(peripheral.name+" ("+peripheral.id+")")).append($("<p>").html(peripheral.uri+"<br/>"+"<i class='zmdi zmdi-bluetooth zmd-fw'></i> "+peripheral.name+" ("+peripheral.id +")"))).append($("<a>",{href:'#dialog',"data-rel":'popup',"data-transition":"pop",onclick:"app.infoPopup('"+peripheral.id+"','ble')"})); 
            else $('li[data-id="'+peripheral.id+'"] a').attr("style","").click(function(){app.go(peripheral.id);});
            $("#devs").html($("#devs li").sort(app.sortRule)).listview("refresh");
          });
        }
      } else $('li[data-id="'+peripheral.id+'"]').addClass("ble").html($("<a>",{href:'#',onclick:"app.uiLoad('"+peripheral.id+"');"}).append($("<img>",{src:"img/ble.svg"})).append($("<h2>").html(peripheral.name+" ("+peripheral.id+")")).append($("<p>").html(peripheral.uri+"<br/>"+"<i class='zmdi zmdi-bluetooth zmd-fw'></i> "+peripheral.name+" ("+peripheral.id +")"))).append($("<a>",{href:'#dialog',"data-rel":'popup',"data-transition":"pop",onclick:"app.infoPopup('"+peripheral.id+"','ble')"})); 
    } else if (app.cachelist[peripheral.id] && app.cachelist[peripheral.id].meta && $('li[data-id="'+peripheral.id+'"]:not(.other)').length==0) {
      $('li.other[data-id="'+peripheral.id+'"]').remove(); 
      $("#other").before($("<li>",{class:"ble item","data-id":peripheral.id,"data-rssi":peripheral.rssi,"data-pplr":(app.cachelist[peripheral.id]).meta.count||Date.now(),"data-time":Date.now()}));
      app.attach(app.cachelist[peripheral.id],(app.cachelist[peripheral.id]).meta);
    } else if (!app.peripherals[peripheral.id]) { 
      if ($('li.other[data-id="'+peripheral.id+'"]').length==0) $(".other:last-child").hide().after($("<li>",{class:"other ble item","data-icon":"false","data-id":peripheral.id,"data-rssi":peripheral.rssi-1000,"data-pplr":Date.now()*1.01,"data-time":Date.now()*1.01}).hide().append($("<a>",{href:"#page2","data-transition":"slide","onclick":"app.generate('"+peripheral.id+"')"}).html("<i class='zmdi zmdi-bluetooth zmd-fw'></i> "+peripheral.name+" ("+peripheral.id +")")));
      if ($("#dv").prop("checked")) $(".other").show();
    }
    $("#devs").html($("#devs li").sort(app.sortRule)).listview("refresh");
    $.mobile.loading("hide");
  ...
  attach: function(peripheral,meta) {
    if (app.meta[meta.url]) {
      if (!$('li[data-id="'+peripheral.id+'"]').attr("data-url")) $('li[data-id="'+peripheral.id+'"]').remove();
      app.meta[meta.url].devices = $.unique(app.meta[meta.url].devices.concat(peripheral.id||peripheral.service.qualifiedname).sort());
      if (app.meta[meta.url].devices.length>1) $('li[data-url="'+meta.url+'"] .n').html(app.meta[meta.url].devices.length + " Devices");
      ...
      if (peripheral.id) $('li[data-id="'+peripheral.id+'"]').attr({"data-url":meta.url,"data-pplr":-meta.count||Date.now()}).html($("<a>",{href:'#',onclick:"app.go('"+peripheral.id+"')"}).append($("<img>",{src:meta.apps.length?meta.apps[0].icon:meta.icon}).error(function(){$(this).attr("src","img/ble.svg")})).append($("<h2>").html(meta.title)).append($("<p>").html(meta.url+"<br/>"+"<i class='zmdi zmdi-bluetooth zmd-fw'></i> <span class='n'>"+peripheral.name+" ("+peripheral.id +")</span>"))).append($("<a>",{href:'#dialog',"data-rel":'popup',"data-transition":"pop",onclick:"app.infoPopup('"+peripheral.id+"','ble')"})).append("<div class='zmdi zmdi-star"+(app.bookmarks[meta.url]?" fav":"")+" zmd-lg' style='float:right; position:relative; z-index:1000' onclick='app.bookmark(\""+meta.url+"\")'></div>");
      else $('li[data-id="'+peripheral.service.qualifiedname+'"]').attr({"data-url":meta.url,"data-pplr":-meta.count||Date.now()}).html($("<a>",{href:'#',onclick:"app.go('"+peripheral.service.qualifiedname+"')"}).append($("<img>",{src:meta.apps.length?meta.apps[0].icon:meta.icon}).error(function(){$(this).attr("src","img/dnssd.svg")})).append($("<h2>").html(meta.title)).append($("<p>").html(meta.url+"<br/>"+"<i class='zmdi zmdi-network-wifi-alt zmd-fw'></i> <span class='n'>"+(peripheral.service.server||peripheral.service.hostName)+" ("+(peripheral.service.application||peripheral.service.type)+")</span>"))).append($("<a>",{href:'#dialog',"data-rel":'popup',"data-transition":"pop",onclick:"app.infoPopup('"+peripheral.service.qualifiedname+"','nsd')"})).append("<div class='zmdi zmdi-star"+(app.bookmarks[meta.url]?" fav":"")+" zmd-lg' style='float:right; position:relative; z-index:1000' onclick='app.bookmark(\""+meta.url+"\")'></div>");
    }
  ...
  bookmark: function(url) {
    console.log("Bookmark " + url + " " + app.bookmarks[url]);
    if (app.bookmarks[url]) {
      delete app.bookmarks[url];
      $("li[data-url='"+url+"'] .zmdi-star").removeClass("fav");
    } else {
      app.bookmarks[url] = app.meta[url] ? app.meta[url].meta.title : "&nbsp;";
      $("li[data-url='"+url+"'] .zmdi-star").addClass("fav");
    }
    localStorage.setItem("bookmarks",JSON.stringify(app.bookmarks));
  },
  sortRule: function(a,b) {
    switch ($("#so").val()) {
      case "time": return ($(b).data('time')) < ($(a).data('time')) ? 1 : -1; break;
      case "rssi": return ($(b).data('rssi')) > ($(a).data('rssi')) ? 1 : -1; break;
      case "pplr": return ($(b).data('pplr')) < ($(a).data('pplr')) ? 1 : -1; break;
    }
  }, 
  ...
  uiLoad: function(id) {
    ...
    $("#lh .rssi").html("("+p.rssi+")");
    $('#lm h4').html('Connecting.').attr("data-id",id);
